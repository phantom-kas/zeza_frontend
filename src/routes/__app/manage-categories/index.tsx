...
function RouteComponent() {
  ...
  const handelSub = async (e) => {
    e.preventDefault()
    setLoading(true)
    await axios.post('/category/update/' + id, { name, description, num_products: productCount }, { _showAllMessages: true }).finally(() => {
      setLoading(false)
    }).then(res => {
  ...
  const handleDroplick = (e, i) => {
    if (e == 'edit') {
      ...
      handleShare(apiBase + '/category/' + i.name + '-' + i.id, i.name, i.name)
    }
    else if (e == 'to') {
      navigate({ to: '/shop', search: { category: i.name } });
    }
  }
  return <> <div className=" relative @container grow pt-10 pb-20">
    <InfiniteLoad onAction={handleDroplick} dropDownOptions={[
      { label: 'Edit Category', emit: 'edit', icon: <Pencil size={16} /> },
