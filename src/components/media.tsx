import { ImageIcon, Pencil, Trash2, VideoIcon } from "lucide-react";
import ConfirmCOmponent from "./confirmCOmponent";
import { ImageCropper } from "./forms/imagePicker";
...
export default () => {
    ...
    const handelDelete = async () => {
        setshowConf(false)
        await axios.delete('/media/delete', { data: { index: selected } }).then(res => {
            if (res.data.status != 'success') return

            queryClient.setQueryData(['systemMediaData'], (oldData: any) => {
    ...
    const handelUpload = async () => {
        ...
        await axios.post(url, data, {
            ...
            .then(res => {
                ...
                queryClient.setQueryData(['systemMediaData'], res.data.data.media)
    ...
    return <><div className=' w-max1200 flex flex-col  mx-auto pb-30'>
        ...
        <div className=' w-full   grid grid-cols-4 max-lg:grid-cols-2 max-sm:grid-cols-1 gap-6 mt-4'>
            {data && data.map((item: { [key: string]: string }, index: number) => {
        ...
        <FullscreenOverlay className=' bg-black p-3 theme1cont w-max1000' isOpen={isOpen} onClose={() => setIsOpen(false)}>
            <ImageCropper quality={1} size={{
                width: 1000, // or desired output width
                height: 1000,
            }} aspectRatio={1.77777778} onCrop={(e) => { handelCrop(e) }} />
        </FullscreenOverlay>
